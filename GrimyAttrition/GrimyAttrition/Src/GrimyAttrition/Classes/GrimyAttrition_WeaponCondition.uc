class GrimyAttrition_WeaponCondition extends X2Condition;
...
event name CallAbilityMeetsCondition(XComGameState_Ability kAbility, XComGameState_BaseObject kTarget)
{
	...
	if (SourceWeapon != none)
	{
		...
		if (WantsReload)
		{
			if ( SourceWeapon.GetClipSize() > 90 ) {
				return 'AA_AmmoUnlimited';	
			}
			else if (SourceWeapon.Ammo == SourceWeapon.GetClipSize()) {
