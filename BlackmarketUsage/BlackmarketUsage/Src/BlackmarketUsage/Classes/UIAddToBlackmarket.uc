...
event OnInit(UIScreen Screen) {
	if ( Screen.IsA('UIBlackMarket_Sell') )
		`LOG("GRIMY LOG IS A BLACK MARKET SELL" $ MyScreenClass);
	else
		`LOG("GRIMY LOG NOT A BLACK MARKET SELL" $ MyScreenClass);
...
defaultproperties
{
	ScreenClass = GrimyLoot_UIBlackmarket_Sell;
}