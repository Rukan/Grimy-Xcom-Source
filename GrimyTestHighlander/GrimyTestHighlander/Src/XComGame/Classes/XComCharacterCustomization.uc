...
{	
	...
	{	
		...
		//Start of Grimy Changes, just commented out this line
...
{	
	...
	local TWeaponAppearance WeaponAppearance, SecondaryWeaponAppearance;	
	...
	case eUICustomizeCat_WeaponColor:
		...
		SecondaryWeaponAppearance = SecondaryWeapon.WeaponAppearance;
		...
		if( specificIndex == -1 ) //default tint is -1 when no tint was ever selected. Keep it -1, else it will push select another tint in the list when player never selected a tint. 
			SecondaryWeaponAppearance.iWeaponTint = -1;
		else
			SecondaryWeaponAppearance.iWeaponTint = WrapIndex(specificIndex, 0, XComHumanPawn(ActorPawn).NumPossibleArmorTints);

		UpdatedUnitState.kAppearance.iWeaponTint = WeaponAppearance.iWeaponTint;
		PrimaryWeapon.WeaponAppearance = WeaponAppearance;
		SecondaryWeapon.WeaponAppearance = SecondaryWeaponAppearance;
	...
	case eUICustomizeCat_WeaponPatterns:
		...
		CyclePartSimple("Patterns", direction, BodyPartFilter.FilterAny, SecondaryWeaponAppearance.nmWeaponPattern, specificIndex);
		...
		if(Items.Length > 0)
		{
			for(Index = 0; Index < Items.Length; ++Index)
			{
				if(XGWeapon(Items[Index].GetVisualizer()) != none)
				{
					if ( Items[Index].InventorySlot == eInvSlot_SecondaryWeapon )
					{
						XGWeapon(Items[Index].GetVisualizer()).SetAppearance(SecondaryWeaponAppearance);
					}
					else
					{
						XGWeapon(Items[Index].GetVisualizer()).SetAppearance(WeaponAppearance);
		...
		{
			XGWeapon(PrimaryWeapon.GetVisualizer()).SetAppearance(WeaponAppearance);
			XGWeapon(SecondaryWeapon.GetVisualizer()).SetAppearance(SecondaryWeaponAppearance);
		}
		...
		if (CosmeticUnitPawn != none)
		{
			Appearance.nmPatterns = SecondaryWeaponAppearance.nmWeaponPattern;
			Appearance.iArmorTint = SecondaryWeaponAppearance.iWeaponTint;
			Appearance.iArmorTintSecondary = SecondaryWeaponAppearance.iWeaponDeco;
			CosmeticUnitPawn.SetAppearance(Appearance, true);
		}
		// end of grimy changes
		UpdatedUnitState.StoreAppearance();
