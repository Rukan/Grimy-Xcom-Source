...
Begin:
	...
	if(ShouldUseLookAtActorCamera() && !bNewUnitSelected)
	{
		...
		LookAtActorCamera.LookAtDuration = min(LookAtDuration,class'GrimyHighlanderConfig'.default.LOOKAT_TIME);
	...
	else if (!bNewUnitSelected)
	{
		...
		LookAtLocationCamera.LookAtDuration = min(LookAtDuration,class'GrimyHighlanderConfig'.default.LOOKAT_TIME);
