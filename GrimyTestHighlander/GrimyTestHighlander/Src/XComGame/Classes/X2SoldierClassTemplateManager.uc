...
function array<SoldierClassAbilityType> GetCrossClassAbilities(optional X2SoldierClassTemplate ExcludeClass)
{
	...
	// Grimy Code, filter out all redundant AWC abilities
	if ( ExcludeClass != none ) {
		for ( iRank = 0; iRank < ExcludeClass.GetMaxConfiguredRank(); iRank++ ) {
			AbilityTree = ExcludeClass.GetAbilityTree(iRank);
			for ( iBranch = 0; iBranch < AbilityTree.Length; iBranch++ ) {
				CrossClassAbilities.RemoveItem(AbilityTree[iBranch]);
			}
		}
	}
