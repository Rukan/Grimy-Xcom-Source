...
simulated function StartTurn() {
	...
	foreach `XCOMHISTORY.IterateByClassType(class'XComGameState_Unit', UnitState, eReturnType_Reference) {
		if (UnitState.GetMyTemplateName() == 'HostileVIPCivilian') {
			ObjArrow = getArrowObject(`XWORLD.GetPositionFromTileCoordinates(UnitState.TileLocation));
			ArrowManager.AddArrowPointingAtActor(UnitState.GetVisualizer(), ObjArrow.Offset, ObjArrow.arrowState, ObjArrow.arrowCounter, class'GotchaUnitFlagHelper'.default.IconSet_Objective_Kill_VIP.defaultIcon);
		}
	}
}
...
private function displaySpottedIcon(UIUnitFlag kFlag, TTile tile, XComGameState_Unit SourceUnitState, XComGameState_Unit TargetUnitState, GameRulesCache_VisibilityInfo VisibilityInfo, bool squadsight, T3DArrow ObjArrow) {
	local EUnitVisibilityState unitVState;
	local bool flanked;
...
private function SetUnitFlagState(UIUnitFlag kFlag, EUnitVisibilityState unitVState, T3DArrow ObjArrow) {
	if (ObjArrow.icon != "")  {
