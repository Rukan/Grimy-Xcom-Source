...
//---------------------------------------------------------------------------------------
class X2AbilityToHitCalc_StandardAim extends X2AbilityToHitCalc config(GameCore) config (GrimyHighlander);
...
// Start of Grimy Additions
var config int MIN_POI_THRESHOLD;
var config bool ENABLE_OVERWATCH_TWEAK;
// End of Grimy Additions
...
protected function int GetHitChance(XComGameState_Ability kAbility, AvailableTarget kTarget, optional bool bDebugLog=false)
{
	...
	{				
		...
		if (bReactionFire)
		{
			if ( !default.ENABLE_OVERWATCH_TWEAK || UnitState.ControllingPlayerIsAI() ||`TACTICALRULES.isAlienActivity() )
			{
...
function float GetReactionAdjust(XComGameState_Unit Shooter, XComGameState_Unit Target)
{
	...
	// Start of Grimy Additions
	if ( default.ENABLE_OVERWATCH_TWEAK && !Shooter.ControllingPlayerIsAI() )
	{
		if ( !`TACTICALRULES.isAlienActivity() )
		{
			return 0;
		}
	}
