class GrimyClassAN_BonusItemCharges extends X2Effect_Persistent config(GrimyClassAN);

var config int DEEP_RESERVES_BONUS;
var int AmmoCount;
	...
	if ( bUtilityGrenades ) {
		foreach ItemStates(ItemState) {
			...
			if ( ItemState.InventorySlot == eInvSlot_Utility && ItemState.GetMyTemplate().IsA('X2GrenadeTemplate') ) {
				...
				if ( UnitState.FindAbility('GrimyAttrition_DeepReserves').ObjectID > 0 ) {
					NewItemState.Ammo += 1;
				}
