...
function bool FilterByTorsoAndArmorMatch(X2BodyPartTemplate Template)
{
	...
	{
		//There cannot be a match if either the template is part of a set and the torso isn't, or vice versa
		if( (TorsoTemplate.SetNames.Length == 0 && Template.SetNames.Length > 0) ||
		    (TorsoTemplate.SetNames.Length > 0 && Template.SetNames.Length == 0) )
		{
			bMatched = false;
		}
