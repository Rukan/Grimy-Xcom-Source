...
simulated function string GetCategoryDisplay(int catType)
{
	...
	case eUICustomizeCat_Arms:              
		...
		break;
	case eUICustomizeCat_LeftArm:
		Result = string(GetCategoryValue("LeftArm", UpdatedUnitState.kAppearance.nmLeftArm, BodyPartFilter.FilterByTorsoAndArmorMatch));
		break;
	case eUICustomizeCat_RightArm:
		Result = string(GetCategoryValue("RightArm", UpdatedUnitState.kAppearance.nmRightArm, BodyPartFilter.FilterByTorsoAndArmorMatch));
		break;
	case eUICustomizeCat_LeftArmDeco:
		Result = string(GetCategoryValue("LeftArmDeco", UpdatedUnitState.kAppearance.nmLeftArmDeco, BodyPartFilter.FilterByTorsoAndArmorMatch));
		break;
	case eUICustomizeCat_RightArmDeco:
		Result = string(GetCategoryValue("RightArmDeco", UpdatedUnitState.kAppearance.nmRightArmDeco, BodyPartFilter.FilterByTorsoAndArmorMatch));
		break;
	case eUICustomizeCat_Legs:              
...
private function GetGenericCategoryList(out array<string> Items,  string BodyPart, delegate<X2BodyPartFilter.FilterCallback> FilterFn, optional string PrefixLocText )
{
	...
	for( i = 0; i < BodyParts.Length; ++i )
	{
		...
		Label = AddIcons(Bodyparts[i]) $ Label;

		Items.AddItem(Label);
	}
}

private function string AddIcons(X2BodyPartTemplate BodyPart)
{
	//local function Label; 

	if( InStr(BodyPart.DataName, "DLC_30", , true) > -1 )
		return class'UIUtilities_Text'.static.InjectImage(class'UIUtilities_Image'.const.HTML_AnarchysChildrenIcon, 26, 26, 0) $ " ";
