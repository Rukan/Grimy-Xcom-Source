class GrimyClassRebalance_AbilitySet_Specialist extends X2Ability_SpecialistAbilitySet;

static function array<X2DataTemplate> CreateTemplates() {
	local array<X2DataTemplate> Templates;

	Templates.AddItem(GrimySupplyProtocol('GrimySupplyProtocol'));
	Templates.AddItem(GrimyTriageProtocol('GrimyTriageProtocol'));
	
	Templates.AddItem(GrimyLootProtocol('GrimyLootProtocol'));

	return Templates;
}

static function X2AbilityTemplate GrimySupplyProtocol(name TemplateName) {
	local X2AbilityTemplate                 Template;

	Template = PurePassive(TemplateName, "img:///UILibrary_PerkIcons.UIPerk_wholenineyards", false, 'eAbilitySource_Perk');
	Template.AdditionalAbilities.AddItem('GrimyLootProtocol');

	return Template;
}

static function X2AbilityTemplate GrimyTriageProtocol(name TemplateName) {
	local X2AbilityTemplate                 Template;

	Template = PurePassive(TemplateName, "img:///UILibrary_PerkIcons.UIPerk_savior", false, 'eAbilitySource_Perk');
...
static function X2AbilityTemplate GrimyLootProtocol(name TemplateName)
{
	local X2AbilityTemplate						Template;		
	local X2AbilityCost_ActionPoints			ActionPointCost;	
	local X2Condition_UnitProperty				UnitPropertyCondition;	
	local GrimyClassRebalance_Condition_Loot	LootableCondition;
	...
	LootableCondition = new class'GrimyClassRebalance_Condition_Loot';
