...
function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState) {
	...
	if (AppliedData.AbilityResultContext.HitResult == eHit_Success || AppliedData.AbilityResultContext.HitResult == eHit_Crit) {
		...
		if ( ItemSlot != eInvSlot_Unknown ) {
			if ( WeaponState.InventorySlot != ItemSlot ) { return 0; }
		}
		else if ( AbilityState.SourceWeapon != EffectState.ApplyEffectParameters.ItemStateObjectRef ) { return 0; }

		if ( bLastStanding && !IsOneOnOne(Attacker, TargetDamageable) ) { return 0; }
...
function bool IsOneOnOne(XComGameState_Unit Attacker, Damageable TargetDamageable) {
	...
	if ( Viewers[0].ObjectID != XComGameState_Unit(TargetDamageable).ObjectID ) { return false; }
	return true;
}
