class GrimyLoot_AbilityCost_Stunned extends X2AbilityCost;
...
simulated function ApplyCost(XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_BaseObject AffectState, XComGameState_Item AffectWeapon, XComGameState NewGameState)
{
	...
	// grimy change here, just add an extra action point after consuming
	AbilityTargetUnit = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID));
	if ( AbilityTargetUnit.IsImpaired(false) && AbilityTargetUnit.IsAlive() ) {
		ModifiedUnitState.ActionPoints.AddItem(RefundActionPoint);
	}	
}
